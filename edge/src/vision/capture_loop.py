...
class CaptureLoop:
    ...
    def process_frame(
        ...
        if frame_has_face:
            ...
            threshold = (
                THRESHOLD_STALE_TEMPLATE if is_stale_template else THRESHOLD_DEFAULT
            )

            if match_confidence >= threshold:
                logging.info(
                    f"Match Successful (Conf: {match_confidence}, Stale: {is_stale_template})"
                )
